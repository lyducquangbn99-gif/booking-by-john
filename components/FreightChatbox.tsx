...
function ContactLink({ href, label, detail, icon, channel }: { href: string; label: string; detail: string; icon: React.ReactNode; channel: "whatsapp" | "zalo" | "email" }) { return <a href={href} target="_blank" rel="noopener noreferrer" onClick={() => trackBookingEvent("contact_click", { contact_channel: channel, contact_placement: "chatbox" })} className="flex items-center gap-3 rounded-xl border border-border-subtle p-3.5 hover:border-ocean-blue hover:bg-bg-primary"><span className="text-ocean-blue">{icon}</span><span><strong className="block">{label}</strong><span className="text-sm text-text-secondary">{detail}</span></span></a>; }
