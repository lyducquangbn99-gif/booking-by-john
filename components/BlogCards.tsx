  ...
  return (
    <section id="blog" className="mx-auto max-w-7xl px-5 py-16 lg:px-8 lg:py-20">
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
        <div>
          <p className="text-sm font-semibold uppercase tracking-wide text-ocean-blue">{t("label")}</p>
        ...
        <Link
          href="/blog"
          className="inline-flex rounded-md border border-border-subtle bg-white px-5 py-3 text-sm font-semibold text-[#0B1F3A] transition hover:border-ocean-blue hover:text-ocean-blue"
        >
          {t("viewAll")}
        </Link>
      </div>
      {posts.length > 0 ? <div className="mt-10 grid gap-5 md:grid-cols-3">
        {posts.map((post) => (
          ...
          >
            <p className="text-xs font-semibold uppercase tracking-wide text-ocean-blue">{post.category}</p>
            <h3 className="mt-4 text-lg font-bold leading-7 text-[#0B1F3A] group-hover:text-ocean-blue">
              {post.title}
            </h3>
            <p className="mt-3 text-sm leading-6 text-text-secondary">{post.excerpt}</p>
            <p className="mt-5 text-sm font-semibold text-accent-orange">{t("readArticle")}</p>
          </Link>
        ))}
